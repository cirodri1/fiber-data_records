---
title: "Analysis of Alpha and Beta diversity in R - rarefaction, alpha- and beta- diversity and vizualizations Hooda_2012_V4V6_single_only-polydextrose"
---
...
```{r}
setwd("/Volumes/Seagate/Meta_analysis/Alpha_Beta_analysis/Hooda_2012_V4V6/polydextrose/")
```
...
* 2. *Metadata* -  **Hooda_metadata_polydextrose.txt** is the Metadata file containing the description of each sample.
...
#### Import metadata:
```{r}
setwd("/Volumes/Seagate/Meta_analysis/Alpha_Beta_analysis/Hooda_2012_V4V6/Input_files")

metadata <- as.data.frame(fread("1.Hooda_metadata_polydextrose.tsv"))
...
#### Subset samples in polydextrose treatment only based on metadata:
```{r}
#Subset the samples in the inulin fiber intervention only:
SV.table_polydextrose <- SV.table %>% select(metadata$sampleid2)
```

#### Save the subsetted data:
```{r}
setwd("/Volumes/Seagate/Meta_analysis/Alpha_Beta_analysis/Hooda_2012_V4V6/polydextrose/Output_files")

#Save subset of samples in a file:
write.table(SV.table_polydextrose, file = "1.SV.table_polydextrose.tsv", sep = "\t", row.names = TRUE, col.names = NA)
```
...
```{r}
SV.table_polydextrose <- as.data.frame(t(SV.table_polydextrose))
```

#### Look at the first few rows and columns to ensure data got transposed:
```{r}
SV.table_polydextrose[1:5,1:5]
```
...
SV.table_polydextrose <- SV.table_polydextrose[, which(colSums(SV.table_polydextrose) != 0)]

SV.combined_polydextrose <- SV.table_polydextrose[, which(colSums(SV.table_polydextrose) != 0)]
```

#### See the total number of samples and SVs, by looking at the dimensions of the table:
```{r}
dim(SV.table_polydextrose)
```
##### Results:
  R always uses rows, then columns. So in this case we have and 19 samples and 412 SVs.

#### If we now want to know the total number of reads we have we can just count up the total of the SVs sum in table:
```{r}
sum(SV.table_polydextrose)
```
##### Results:
   We have 94676 reads.
...
```{r}
reads.per.sample <- rowSums(SV.table_polydextrose) %>% sort()
reads.per.sample
...
rowSums(SV.table_polydextrose) %>% summary()
colSums(SV.table_polydextrose) %>% summary()
```
#### Result:
* The min. number of sequences in a sample is 2966 and the max. is 10781; we can use 2966 for rarefaction depth.
 * The min. value for the presence of an SV is 2 and the max. times an SV is present is 12135.

#### Performing the actual rarefaction 
* Rarefaction depth should be >= 1000 at least, we will use the minimum value of sequence depth which is 2966 
* Round out the averages, so we will not have exactly 2966 sequences in each, but we also will not be keeping the super rare, super low abundance << 1 taxa https://rdrr.io/github/GuillemSalazar/EcolUtils/man/rrarefy.perm.html
```{r, results='hide'}
set.seed(2022) #reproducible results
SV.table.rarefied <- as.data.frame((rrarefy.perm(SV.table_polydextrose, sample = 2966, n = 1000, round.out = TRUE)))
...
write.table(SV.table.rarefied, file = "3.polydextrose_OTU_filtered_rarefied_2966_ROUNDED.tsv", sep = "\t", row.names = TRUE, col.names = NA)
```
...
write.table(x = merged_alpha, file = "4.polydextrose_alpha_metrics_merged_metadata.txt", sep = "\t", row.names = TRUE, col.names = NA) 
...
ggplot (merged_alpha, aes(x= factor (timepoint, level= c("before","after")), y=Shannon)) +
  ...
  labs(title = "Shannon-alpha diversity boxplot / Hooda_2012_V4V6-polydextrose", x= "Timepoint in fiber treatment", y= "Shannon alpha diversity", fill="Timepoint") +
...
ggplot (merged_alpha, aes(x= factor (timepoint, level= c("before","after")), y=Simpson)) +
  ...
  scale_fill_hue(limits = c ("before","after")) +
  geom_line(aes(group = `subject_id`), color="grey") +
  labs(title = "Simpson-alpha diversity boxplot / Hooda_2012_V4V6-polydextrose", x= "Timepoint in fiber treatment", y= "Simpson alpha diversity", fill="Timepoint") +
  theme_bw (base_family = "Tw Cen MT", base_size = 14) +
  stat_summary(fun = mean, geom = "point", shape=23, size=2.5, fill="black")
```
...
write.table(x = pairs.only.all.alpha, file = "Output_files/6.polydextrose_alpha_metrics_merged_metadata_only_pairs.txt", sep = "\t", row.names = TRUE, col.names = NA) 
...
* Are before fiber samples more/less diverse than after fiber samples?
    - ANSWER: Based on the plots, it seems that alpha diversity slightly increases
 
* Does diversity change after treatments?
    - ANSWER: Based on the mean alpha diversity metrics, it seems that after the intervention the diversity slighty increases
...
bray.dist <- avgdist(SV.table_polydextrose, 
                     sample = 2966, 
...
labs(title = "MDS Hooda_2012_V4V6 polydextrose", x= "MDS1", y= "MDS2", fill="Timepoint") +
...
```{r}
set.seed(2022) #reproducible results
  bray.dist.matrix <- as.data.frame(as.matrix(avgdist(SV.table_polydextrose, sample = 2966, meanfun = median, transf = sqrt, iterations = 1000))) #default is Bray-Curtis calculated by vegdist()
```

#### Save Bray-Curtis dissimilarity matrix in a file:
```{r}
  write.table(x = bray.dist.matrix, file = "Output_files/9.polydextrose_median_bray_curtis_sqrt_q1_single_rarefied_to_2966.txt", sep = "\t", row.names = TRUE, col.names = NA)  
...
* timepoint (before vs after) =  YES significant, explaining 4.6% of variance
