---
title: "Betadisper Venkataraman_potato"
---
...
```{r}
setwd("/Volumes/Seagate_Cedars/Martiny_Lab/Meta-analysis/Data_analysis/Meta_analysis/Betadisp/Venkataraman_2016_potato")
```

## __________ Documents needed: __________##

* 1. *beta_matrix* - **9.potato_median_bray_curtis_sqrt_q1_single_rarefied_to_2167.txt**

* 2. *metadata* - **1.Venkataraman_metadata_potato.tsv** 
...
beta_matrix <- as.data.frame(read.table("Input/9.potato_median_bray_curtis_sqrt_q1_single_rarefied_to_2167.txt", header = TRUE, row.names=1))


metadata <- as.data.frame(fread("Input/1.Venkataraman_metadata_potato.tsv", header = TRUE))
```
...
```{r}
nmds_plus_metadata <- merge(beta_coordinates, metadata, by.x = "V1", by.y = "sample_id_2")
...
labs(title = "NMDS Venkataraman_potato", x= "MDS1", y= "MDS2", fill="Timepoint") +
...
#### Save figure:
```{r}
ggsave("Figures/NMDS.Venkataraman_potato.png", width = 7, height = 5)
```
