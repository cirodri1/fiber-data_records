...
```{r}
setwd("/Volumes/Seagate_Cedars/Martiny_Lab/Meta-analysis/Data_analysis/Meta_analysis/Betadisp/Dahl_2016_RS4A")
```

## __________ Documents needed: __________##

* 1. *beta_matrix* - **9.RS4A_median_bray_curtis_sqrt_q1_single_rarefied_to_16289.txt** 

* 2. *metadata* - **1.Dahl_metadata_RS4A.tsv** 
...
beta_matrix <- as.data.frame(read.table("Input/9.RS4A_median_bray_curtis_sqrt_q1_single_rarefied_to_16289.txt", header = TRUE, row.names=1))


metadata <- as.data.frame(fread("Input/1.Dahl_metadata_RS4A.tsv", header = TRUE))
```
...
labs(title = "NMDS Dahl_RS4A", x= "MDS1", y= "MDS2", fill="Timepoint") +
...
#### Save figure:
```{r}
ggsave("Figures/NMDS.Dahl_RS4Ae.png", width = 7, height = 5)
```
