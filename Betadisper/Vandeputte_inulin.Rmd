---
title: "Betadisper Vandeputte_inulin"
---
...
```{r}
setwd("/Volumes/Seagate_Cedars/Martiny_Lab/Meta-analysis/Data_analysis/Meta_analysis/Betadisp/Vandeputte_2017_inulin")
```

## __________ Documents needed: __________##

* 1. *beta_matrix* - **9.inulin_median_bray_curtis_sqrt_q1_single_rarefied_to_7912.txt**

* 2. *metadata* - **1.vandeputte_metadata_inulin.tsv** 
...
beta_matrix <- as.data.frame(read.table("Input/9.inulin_median_bray_curtis_sqrt_q1_single_rarefied_to_7912.txt", header = TRUE, row.names=1))


metadata <- as.data.frame(fread("Input/1.vandeputte_metadata_inulin.tsv", header = TRUE))
```
...
labs(title = "NMDS Vandeputte_inulin", x= "MDS1", y= "MDS2", fill="Timepoint") +
...
#### Save figure:
```{r}
ggsave("Figures/NMDS.Vandeputte_inulin.png", width = 7, height = 5)
```
