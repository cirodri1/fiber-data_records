---
title: "Betadisper Rasmussen_psyllium"
---
...
```{r}
setwd("/Volumes/Seagate_Cedars/Martiny_Lab/Meta-analysis/Data_analysis/Meta_analysis/Betadisp/Rasmussen_2017_psyllium")
```

## __________ Documents needed: __________##

* 1. *beta_matrix* - **9.psyllium_median_bray_curtis_sqrt_q1_single_rarefied_to_1197.txt**

* 2. *metadata* - **1.Rasmussen_metadata_psyllium.tsv** 
...
beta_matrix <- as.data.frame(read.table("Input/9.psyllium_median_bray_curtis_sqrt_q1_single_rarefied_to_1197.txt", header = TRUE, row.names=1))


metadata <- as.data.frame(fread("Input/1.Rasmussen_metadata_psyllium.tsv", header = TRUE))
```
...
labs(title = "NMDS Rasmussen_psyllium", x= "MDS1", y= "MDS2", fill="Timepoint") +
...
#### Save figure:
```{r}
ggsave("Figures/NMDS.Rasmussen_psyllium.png", width = 7, height = 5)
```
