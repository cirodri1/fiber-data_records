---
title: "Analysis of Alpha and Beta diversity in R - rarefaction, alpha- and beta- diversity and vizualizations Morales_2016_V3V4_forward_only-oligofructose"
---
...
```{r}
setwd("/Volumes/Seagate/Meta_analysis/Alpha_Beta_analysis/Morales_2016_V3V4/oligofructose/")
```
...
* 2. *Metadata* -  **Morales_metadata_oligofructose.txt** is the Metadata file containing the description of each sample.
...
#### Import metadata:
```{r}
setwd("/Volumes/Seagate/Meta_analysis/Alpha_Beta_analysis/Morales_2016_V3V4/Input_files")

metadata <- as.data.frame(fread("1.Morales_metadata_oligofructose.tsv"))
...
SV.table_oligofructose <- SV.table %>% select(metadata$sampleid2)
```

#### Save the subsetted data:
```{r}
setwd("/Volumes/Seagate/Meta_analysis/Alpha_Beta_analysis/Morales_2016_V3V4/oligofructose/Output_files")

#Save subset of samples in a file:
write.table(SV.table_oligofructose, file = "1.SV.table_oligofructose.tsv", sep = "\t", row.names = TRUE, col.names = NA)
```
...
```{r}
SV.table_oligofructose <- as.data.frame(t(SV.table_oligofructose))
```

#### Look at the first few rows and columns to ensure data got transposed:
```{r}
SV.table_oligofructose[1:5,1:5]
```
...
SV.table_oligofructose <- SV.table_oligofructose[, which(colSums(SV.table_oligofructose) != 0)]

SV.combined_oligofructose <- SV.table_oligofructose[, which(colSums(SV.table_oligofructose) != 0)]
```

#### See the total number of samples and SVs, by looking at the dimensions of the table:
```{r}
dim(SV.table_oligofructose)
```
##### Results:
  R always uses rows, then columns. So in this case we have and 22 samples and 1730 SVs.

#### If we now want to know the total number of reads we have we can just count up the total of the SVs sum in table:
```{r}
sum(SV.table_oligofructose)
```
##### Results:
   We have 2188402 reads.


#### To see the number of reads per sample, we simply need to add up the rows on the file:
```{r}
reads.per.sample <- rowSums(SV.table_oligofructose) %>% sort()
reads.per.sample
...
rowSums(SV.table_oligofructose) %>% summary()
colSums(SV.table_oligofructose) %>% summary()
```
#### Result:
* The min. number of sequences in a sample is 66061 and the max. is 124036; we can use 66061 for rarefaction depth, we will lose 1 sample.
 * The min. value for the presence of an SV is 2 and the max. times an SV is present is 229095.

#### Performing the actual rarefaction 
* Rarefaction depth should be >= 1000 at least, we will use the minimum value of sequence depth which is 66061 
* Round out the averages, so we will not have exactly 66061 sequences in each, but we also will not be keeping the super rare, super low abundance << 1 taxa https://rdrr.io/github/GuillemSalazar/EcolUtils/man/rrarefy.perm.html
```{r, results='hide'}
set.seed(2022) #reproducible results
SV.table.rarefied <- as.data.frame((rrarefy.perm(SV.table_oligofructose, sample = 66061, n = 1000, round.out = TRUE)))
...
write.table(SV.table.rarefied, file = "3.oligofructose_OTU_filtered_rarefied_66061_ROUNDED.tsv", sep = "\t", row.names = TRUE, col.names = NA)
```
...
write.table(x = merged_alpha, file = "4.oligofructose_alpha_metrics_merged_metadata.txt", sep = "\t", row.names = TRUE, col.names = NA) 
...
ggplot (merged_alpha, aes(x= factor (timepoint, level= c("before","after")), y=Shannon)) +
  ...
  labs(title = "Shannon-alpha diversity boxplot / Morales_2016_V3V4-oligofructose", x= "Timepoint in fiber treatment", y= "Shannon alpha diversity", fill="Timepoint") +
...
ggplot (merged_alpha, aes(x= factor (timepoint, level= c("before","after")), y=Simpson)) +
  ...
  scale_fill_hue(limits = c ("before","after")) +
  geom_line(aes(group = `subject_id`), color="grey") +
  labs(title = "Simpson-alpha diversity boxplot / Morales_2016_V3V4-oligofructose", x= "Timepoint in fiber treatment", y= "Simpson alpha diversity", fill="Timepoint") +
  theme_bw (base_family = "Tw Cen MT", base_size = 14) +
  stat_summary(fun = mean, geom = "point", shape=23, size=2.5, fill="black")
```
...
write.table(x = pairs.only.all.alpha, file = "Output_files/6.oligofructose_alpha_metrics_merged_metadata_only_pairs.txt", sep = "\t", row.names = TRUE, col.names = NA) 
...
bray.dist <- avgdist(SV.table_oligofructose, 
                     sample = 66061, 
...
labs(title = "MDS Morales_2016_V3V4 oligofructose", x= "MDS1", y= "NMDS2", fill="Timepoint") +
...
```{r}
set.seed(2022) #reproducible results
  bray.dist.matrix <- as.data.frame(as.matrix(avgdist(SV.table_oligofructose, sample = 66061, meanfun = median, transf = sqrt, iterations = 1000))) #default is Bray-Curtis calculated by vegdist()
```

#### Save Bray-Curtis dissimilarity matrix in a file:
```{r}
  write.table(x = bray.dist.matrix, file = "Output_files/9.oligofructose_median_bray_curtis_sqrt_q1_forward_rarefied_to_66061.txt", sep = "\t", row.names = TRUE, col.names = NA)  
