---
title: "Analysis of Alpha and Beta diversity in R - rarefaction, alpha- and beta- diversity and vizualizations Baxter_2019_V4_forward_only-himaize"
---
...
```{r}
setwd("/Volumes/Seagate/Meta_analysis/Alpha_Beta_analysis/Baxter_2019_V4/himaize/")
```
...
* 2. *Metadata* -  **Baxter_metadata_himaize.txt** is the Metadata file containing the description of each sample.
...
#### Import metadata: Note: Sample SRR6444629 from subject U074 was modified from timepoint 8 to 7, and sample SRR6444610 (from same subject) was moved from 9 to 8. Sample SRR6443876 deleted because two tiempoint 8s.
```{r}
setwd("/Volumes/Seagate/Meta_analysis/Alpha_Beta_analysis/Baxter_2019_V4/Input_files")

metadata <- as.data.frame(fread("1.Baxter_metadata_himaize.tsv"))
...
#### Subset samples in himaize treatment only based on metadata:
```{r}
#Subset the samples in the himaize fiber intervention only:
SV.table_himaize <- SV.table %>% select(metadata$sampleid2)

#Save subset of samples in a file:
write.table(SV.table_himaize, file = "Output_files/1.SV.table_himaize.tsv", sep = "\t", row.names = TRUE, col.names = NA)
```
...
```{r}
SV.table_himaize <- as.data.frame(t(SV.table_himaize))
```

#### Look at the first few rows and columns to ensure data got transposed:
```{r}
SV.table_himaize[1:5,1:5]
```
...
SV.table_himaize <- SV.table_himaize[, which(colSums(SV.table_himaize) != 0)]

```

#### See the total number of samples and SVs, by looking at the dimensions of the table:
```{r}
dim(SV.table_himaize)
```
##### Results:
  R always uses rows, then columns. So in this case we have and 313 samples and 3629 SVs.

#### If we now want to know the total number of reads we have we can just count up the total of the SVs sum in table:
```{r}
sum(SV.table_himaize)
```
##### Results:
   We have 7072923 reads.


#### To see the number of reads per sample, we simply need to add up the rows on the file:
```{r}
reads.per.sample <- rowSums(SV.table_himaize) %>% sort()
reads.per.sample

#Save a file with the information of reads per sample:
setwd("/Volumes/Seagate/Meta_analysis/Alpha_Beta_analysis/Baxter_2019_V4/himaize/Output_files")
...
rowSums(SV.table_himaize) %>% summary()
colSums(SV.table_himaize) %>% summary()
```
#### Result:
 * The min. number of sequences in a sample are 4891 and the max. is 134728; we can use 4891 for rarefaction depth.
 * The min. value for the presence of an SV is 2 and the max. times an SV is present is 787145

#### Performing the actual rarefaction 
* Rarefaction depth should be >= 1000 at least, we will use the minimum value of sequence depth which is 4891 
* Round out the averages, so we will not have exactly 4891 sequences in each, but we also will not be keeping the super rare, super low abundance << 1 taxa https://rdrr.io/github/GuillemSalazar/EcolUtils/man/rrarefy.perm.html
```{r, results='hide'}
set.seed(2022) #reproducible results
SV.table.rarefied <- as.data.frame((rrarefy.perm(SV.table_himaize, sample = 4891, n = 1000, round.out = TRUE)))
...
write.table(SV.table.rarefied, file = "3.himaize_OTU_filtered_rarefied_4891_ROUNDED.tsv", sep = "\t", row.names = TRUE, col.names = NA)
```
...
write.table(x = merged_alpha, file = "4.himaize_alpha_metrics_merged_metadata.txt", sep = "\t", row.names = TRUE, col.names = NA) 
...
ggplot (merged_alpha, aes(x= factor (timepoint, level= c("before","after")), y=Shannon)) +
  ...
  labs(title = "Shannon-alpha diversity boxplot / Baxter_2019_V4-himaize", x= "Timepoint in fiber treatment", y= "Shannon alpha diversity", fill="Timepoint") +
...
ggplot (merged_alpha, aes(x= factor (timepoint, level= c("before","after")), y=Simpson)) +
  ...
  scale_fill_hue(limits = c ("before","after")) +
  geom_line(aes(group = `subject_id`), color="grey") +
  labs(title = "Simpson-alpha diversity boxplot / Baxter_2019_V4-himaize", x= "Timepoint in fiber treatment", y= "Simpson alpha diversity", fill="Timepoint") +
  theme_bw (base_family = "Tw Cen MT", base_size = 14) +
  stat_summary(fun = mean, geom = "point", shape=23, size=2.5, fill="black")
```
...
write.table(x = merged_alpha_2, file = "5.subsetted_himaize_alpha_metrics_merged_metadata.txt", sep = "\t", row.names = TRUE, col.names = NA) 
...
#Save file with only before and during subjects, any subjects that did not have both timepoints were deleted:
write.table(x = pairs.only.all.alpha, file = "Output_files/6.himaize_alpha_metrics_merged_metadata_only_pairs.txt", sep = "\t", row.names = TRUE, col.names = NA) 
...
* Are before fiber samples more/less diverse than after fiber samples?
    - ANSWER: Based on the plots, it seems that alpha diversity decreases after fiber treatment
...
bray.dist <- avgdist(SV.table_himaize, 
                     sample = 4891, 
...
#Save Bray-Curtis dissimilarity NMDS cordinates plus metadata:
  write.table(x = nmds_plus_metadata, file = "Output_files/8.NMDS.coordinates.metadata.txt", sep = "\t", row.names = TRUE, col.names = NA) 
...
labs(title = "MDS Baxter_2019_V4 himaize", x= "MDS1", y= "MDS2", fill="Timepoint") +
...
```{r}
set.seed(2022) #reproducible results
  bray.dist.matrix <- as.data.frame(as.matrix(avgdist(SV.table_himaize, sample = 4891, meanfun = median, transf = sqrt, iterations = 1000))) #default is Bray-Curtis calculated by vegdist()
```

#### Save Bray-Curtis dissimilarity matrix in a file:
```{r}
  write.table(x = bray.dist.matrix, file = "Output_files/9.himaize_median_bray_curtis_sqrt_q1_single_rarefied_to_4891.txt", sep = "\t", row.names = TRUE, col.names = NA)  
...
* subject_id =  YES significant, explaining 86% of variance
* timepoint (before vs after) =  YES significant, explaining 0.2% of variance
