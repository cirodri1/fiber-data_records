---
title: "Analysis of Alpha and Beta diversity in R - rarefaction, alpha- and beta- diversity and vizualizations Tap_2015_V3V4_single_only-dietary_fiber"
---
...
```{r}
setwd("/Volumes/Seagate/Meta_analysis/Alpha_Beta_analysis/Tap_2015_V3V4/dietary_fiber/")
```
...
* 2. *Metadata* -  **tap_metadata_dietary_fiber.txt** is the Metadata file containing the description of each sample.
...
#### Import metadata:
```{r}
setwd("/Volumes/Seagate/Meta_analysis/Alpha_Beta_analysis/Tap_2015_V3V4/Input_files")

metadata <- as.data.frame(fread("1.tap_metadata.tsv"))
...
SV.table_fiber <- SV.table %>% select(metadata$sampleid2)
```

#### Save the subsetted data:
```{r}
setwd("/Volumes/Seagate/Meta_analysis/Alpha_Beta_analysis/Tap_2015_V3V4/dietary_fiber/Output_files")

#Save subset of samples in a file:
write.table(SV.table_fiber, file = "1.SV.table_fiber.tsv", sep = "\t", row.names = TRUE, col.names = NA)
```
...
SV.table_fiber <- SV.table_fiber[, which(colSums(SV.table_fiber) != 0)]

SV.combined_fiber <- SV.table_fiber[, which(colSums(SV.table_fiber) != 0)]
```
...
```
##### Results:
  R always uses rows, then columns. So in this case we have and 38 samples and 962 SVs.
...
rowSums(SV.table_fiber) %>% summary()
colSums(SV.table_fiber) %>% summary()
```
#### Result:
* The min. number of sequences in a sample are 17 and the max. is 10969; since 17 is too low, we can use 1021 for rarefaction depth, we will lose 1 sample.
 * The min. value for the presence of an SV is 2 and the max. times an SV is present is 9317.

#### Performing the actual rarefaction 
* Rarefaction depth should be >= 1000 at least, we will use the minimum value of sequence depth which is 1021 
* Round out the averages, so we will not have exactly 1021 sequences in each, but we also will not be keeping the super rare, super low abundance << 1 taxa https://rdrr.io/github/GuillemSalazar/EcolUtils/man/rrarefy.perm.html
```{r, results='hide'}
set.seed(2022) #reproducible results
SV.table.rarefied <- as.data.frame((rrarefy.perm(SV.table_fiber, sample = 1021, n = 1000, round.out = TRUE)))
...
#### Drop those samples that had under rarefaction threshold (samples ERR012069,ERR012025,ERR012047,ERR012064):
#https://www.datasciencemadesimple.com/delete-or-drop-rows-in-r-with-conditions-2/
```{r}
 SV.table.rarefied <- SV.table.rarefied[!(row.names(SV.table.rarefied) %in% c('ERR012069')), ]

sort(rowSums(SV.table.rarefied))
```
#### Results:
We dropped sample ERR012069

#### Save filtered & rarefied OTU SV.table.rarefied:
```{r}
setwd("/Volumes/Seagate/Meta_analysis/Alpha_Beta_analysis/Tap_2015_V3V4/dietary_fiber/Output_files")

write.table(SV.table.rarefied, file = "3.dietary_fiber_OTU_filtered_rarefied_1021_ROUNDED.tsv", sep = "\t", row.names = TRUE, col.names = NA)
```
...
write.table(x = merged_alpha, file = "4.dietary_fiber_alpha_metrics_merged_metadata.txt", sep = "\t", row.names = TRUE, col.names = NA) 
...
ggplot (merged_alpha, aes(x= factor (timepoint, level= c("before","after")), y=Shannon)) +
  ...
  labs(title = "Shannon-alpha diversity boxplot / Tap_2015_V3V4-dietary_fiber", x= "Timepoint in fiber treatment", y= "Shannon alpha diversity", fill="Timepoint") +
...
ggplot (merged_alpha, aes(x= factor (timepoint, level= c("before","after")), y=Simpson)) +
  ...
  scale_fill_hue(limits = c ("before","after")) +
  geom_line(aes(group = `subject_id`), color="grey") +
  labs(title = "Simpson-alpha diversity boxplot / Tap_2015_V3V4-dietary_fiber", x= "Timepoint in fiber treatment", y= "Simpson alpha diversity", fill="Timepoint") +
  theme_bw (base_family = "Tw Cen MT", base_size = 14) +
  stat_summary(fun = mean, geom = "point", shape=23, size=2.5, fill="black")
```
...
write.table(x = pairs.only.all.alpha, file = "Output_files/6.dietary_fiber_alpha_metrics_merged_metadata_only_pairs.txt", sep = "\t", row.names = TRUE, col.names = NA) 
...
bray.dist <- avgdist(SV.table_fiber, 
                     sample = 1021, 
...
labs(title = "MDS Tap_2015_V3V4 dietary_fiber", x= "MDS1", y= "NMDS2", fill="Timepoint") +
...
```{r}
set.seed(2022) #reproducible results
  bray.dist.matrix <- as.data.frame(as.matrix(avgdist(SV.table_fiber, sample = 1021, meanfun = median, transf = sqrt, iterations = 1000))) #default is Bray-Curtis calculated by vegdist()
```

#### Save Bray-Curtis dissimilarity matrix in a file:
```{r}
  write.table(x = bray.dist.matrix, file = "Output_files/9.dietary_fiber_median_bray_curtis_sqrt_q1_single_rarefied_to_1021.txt", sep = "\t", row.names = TRUE, col.names = NA)  
...
```{r}
metadata <- metadata[!(metadata$sampleid2 %in% c('ERR012069')), ]
...
* subject_id =  YES significant, explaining 71% of variance
* timepoint (before vs after) =  NOT significant, explaining 1.4% of variance
