---
title: "Betadisper Kovatcheva_bread"
---
...
```{r}
setwd("/Volumes/Seagate_Cedars/Martiny_Lab/Meta-analysis/Data_analysis/Meta_analysis/Betadisp/Kovatcheva_2015_bread")
```

## __________ Documents needed: __________##

* 1. *beta_matrix* - **9.bread_median_bray_curtis_sqrt_q1_single_rarefied_to_3642.txt** 

* 2. *metadata* - **1.Kovatcheva_metadata_bread.tsv** 
...
beta_matrix <- as.data.frame(read.table("Input/9.bread_median_bray_curtis_sqrt_q1_single_rarefied_to_3642.txt", header = TRUE, row.names=1))


metadata <- as.data.frame(fread("Input/1.Kovatcheva_metadata_bread.tsv", header = TRUE))
```
...
labs(title = "NMDS Kovatcheva_bread", x= "MDS1", y= "MDS2", fill="Timepoint") +
...
#### Save figure:
```{r}
ggsave("Figures/NMDS.Kovatcheva_bread.png", width = 7, height = 5)
```
