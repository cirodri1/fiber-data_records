---
title: "Analysis of Alpha and Beta diversity in R - rarefaction, alpha- and beta- diversity and vizualizations Rasmussen_2017_V1V3_single_only-SM12"
---
...
```{r}
setwd("/Volumes/Seagate/Meta_analysis/Alpha_Beta_analysis/Rasmussen_2017_V1V3/SM12/")
```
...
* 2. *Metadata* -  **Rasmussen_metadata_SM12.txt** is the Metadata file containing the description of each sample.
...
metadata <- as.data.frame(fread("1.Rasmussen_metadata_SM12.tsv"))
...
SV.table_SM12 <- SV.table %>% select(metadata$sampleid2)
```

#### Save the subsetted data:
```{r}
setwd("/Volumes/Seagate/Meta_analysis/Alpha_Beta_analysis/Rasmussen_2017_V1V3/SM12/Output_files")

#Save subset of samples in a file:
write.table(SV.table_SM12, file = "1.SV.table_SM12.tsv", sep = "\t", row.names = TRUE, col.names = NA)
```
...
SV.table_SM12 <- SV.table_SM12[, which(colSums(SV.table_SM12) != 0)]

SV.combined_SM12 <- SV.table_SM12[, which(colSums(SV.table_SM12) != 0)]
```
...
```
##### Results:
  R always uses rows, then columns. So in this case we have and 30 samples and 1302 SVs.
...
rowSums(SV.table_SM12) %>% summary()
colSums(SV.table_SM12) %>% summary()
```
#### Result:
* The min. number of sequences in a sample are 4 and the max. is 12057; since 4 is too low, we can use 3217 for rarefaction depth, we will lose 1 sample.
 * The min. value for the presence of an SV is 2 and the max. times an SV is present is 13075.

#### Performing the actual rarefaction 
* Rarefaction depth should be >= 1000 at least, we will use the minimum value of sequence depth which is 3217 
* Round out the averages, so we will not have exactly 3217 sequences in each, but we also will not be keeping the super rare, super low abundance << 1 taxa https://rdrr.io/github/GuillemSalazar/EcolUtils/man/rrarefy.perm.html
```{r, results='hide'}
set.seed(2022) #reproducible results
SV.table.rarefied <- as.data.frame((rrarefy.perm(SV.table_SM12, sample = 3217, n = 1000, round.out = TRUE)))
...
#### Drop those samples that had under rarefaction threshold (samples SRR5504403):
#https://www.datasciencemadesimple.com/delete-or-drop-rows-in-r-with-conditions-2/
```{r}
 SV.table.rarefied <- SV.table.rarefied[!(row.names(SV.table.rarefied) %in% c('SRR5504403')), ]
...
write.table(SV.table.rarefied, file = "3.SM12_OTU_filtered_rarefied_3217_ROUNDED.tsv", sep = "\t", row.names = TRUE, col.names = NA)
```
...
write.table(x = merged_alpha, file = "4.SM12_alpha_metrics_merged_metadata.txt", sep = "\t", row.names = TRUE, col.names = NA) 
...
ggplot (merged_alpha, aes(x= factor (timepoint, level= c("before","after")), y=Shannon)) +
  ...
  labs(title = "Shannon-alpha diversity boxplot / Rasmussen_2017_V1V3-SM12", x= "Timepoint in fiber treatment", y= "Shannon alpha diversity", fill="Timepoint") +
...
ggplot (merged_alpha, aes(x= factor (timepoint, level= c("before","after")), y=Simpson)) +
  ...
  scale_fill_hue(limits = c ("before","after")) +
  geom_line(aes(group = `subject_id`), color="grey") +
  labs(title = "Simpson-alpha diversity boxplot / Rasmussen_2017_V1V3-SM12", x= "Timepoint in fiber treatment", y= "Simpson alpha diversity", fill="Timepoint") +
  theme_bw (base_family = "Tw Cen MT", base_size = 14) +
  stat_summary(fun = mean, geom = "point", shape=23, size=2.5, fill="black")
```
...
write.table(x = pairs.only.all.alpha, file = "Output_files/6.SM12_alpha_metrics_merged_metadata_only_pairs.txt", sep = "\t", row.names = TRUE, col.names = NA) 
...
bray.dist <- avgdist(SV.table_SM12, 
                     sample = 3217, 
...
labs(title = "MDS Rasmussen_2017_V1V3 SM12", x= "MDS1", y= "NMDS2", fill="Timepoint") +
...
```{r}
set.seed(2022) #reproducible results
  bray.dist.matrix <- as.data.frame(as.matrix(avgdist(SV.table_SM12, sample = 3217, meanfun = median, transf = sqrt, iterations = 1000))) #default is Bray-Curtis calculated by vegdist()
```

#### Save Bray-Curtis dissimilarity matrix in a file:
```{r}
  write.table(x = bray.dist.matrix, file = "Output_files/9.SM12_median_bray_curtis_sqrt_q1_single_rarefied_to_3217.txt", sep = "\t", row.names = TRUE, col.names = NA)  
...
* subject_id =  YES significant, explaining 74% of variance
* timepoint (before vs after) =  NOT significant, explaining 1.6% of variance
