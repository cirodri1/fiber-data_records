---
title: "Analysis of Alpha and Beta diversity in R - rarefaction, alpha- and beta- diversity and vizualizations Baxter_2019_V4_forward_only-potato"
---
...
```{r}
setwd("/Volumes/Seagate/Meta_analysis/Alpha_Beta_analysis/Baxter_2019_V4/potato/")
```
...
* 2. *Metadata* -  **Baxter_metadata_potato.txt** is the Metadata file containing the description of each sample.
...
metadata <- as.data.frame(fread("1.Baxter_metadata_potato.tsv"))
...
```{r}
#Subset the samples in the potato fiber intervention only:
SV.table_potato <- SV.table %>% select(metadata$sampleid2)

#Save subset of samples in a file:
write.table(SV.table_potato, file = "Output_files/1.SV.table_potato.tsv", sep = "\t", row.names = TRUE, col.names = NA)
```
...
```
##### Results:
  R always uses rows, then columns. So in this case we have and 273 samples and 3619 SVs.
...
setwd("/Volumes/Seagate/Meta_analysis/Alpha_Beta_analysis/Baxter_2019_V4/potato/Output_files")
...
```
#### Result:
 * The min. number of sequences in a sample are 4622 and the max. is 50476; we can use 4622 for rarefaction depth.
 * The min. value for the presence of an SV is 2 and the max. times an SV is present is 983495.

#### Performing the actual rarefaction 
* Rarefaction depth should be >= 1000 at least, we will use the minimum value of sequence depth which is 4622 
* Round out the averages, so we will not have exactly 4622 sequences in each, but we also will not be keeping the super rare, super low abundance << 1 taxa https://rdrr.io/github/GuillemSalazar/EcolUtils/man/rrarefy.perm.html
```{r, results='hide'}
set.seed(2022) #reproducible results
SV.table.rarefied <- as.data.frame((rrarefy.perm(SV.table_potato, sample = 4622, n = 1000, round.out = TRUE)))
...
write.table(SV.table.rarefied, file = "3.potato_OTU_filtered_rarefied_4622_ROUNDED.tsv", sep = "\t", row.names = TRUE, col.names = NA)
```
...
ggplot (merged_alpha, aes(x= factor (timepoint, level= c("before","after")), y=Shannon)) +
  ...
  labs(title = "Shannon-alpha diversity boxplot / Baxter_2019_V4-potato", x= "Timepoint in fiber treatment", y= "Shannon alpha diversity", fill="Timepoint") +
...
ggplot (merged_alpha, aes(x= factor (timepoint, level= c("before","after")), y=Simpson)) +
  ...
  scale_fill_hue(limits = c ("before","after")) +
  geom_line(aes(group = `subject_id`), color="grey") +
  labs(title = "Simpson-alpha diversity boxplot / Baxter_2019_V4-potato", x= "Timepoint in fiber treatment", y= "Simpson alpha diversity", fill="Timepoint") +
  theme_bw (base_family = "Tw Cen MT", base_size = 14) +
  stat_summary(fun = mean, geom = "point", shape=23, size=2.5, fill="black")
```
...
* Are before fiber samples more/less diverse than after fiber samples?
    - ANSWER: Based on the plots, it seems that alpha diversity slightly decreases after fiber treatment only using Shannon metric
 
* Does diversity change after treatments?
    - ANSWER: Based on the mean alpha diversity metrics, it seems that after the intervention the diversity slightly decreases only using Shannon metric.
...
labs(title = "MDS Baxter_2019_V4 potato", x= "MDS1", y= "MDS2", fill="Timepoint") +
...
```{r}
set.seed(2022) #reproducible results
  bray.dist.matrix <- as.data.frame(as.matrix(avgdist(SV.table_potato, sample = 4622, meanfun = median, transf = sqrt, iterations = 1000))) #default is Bray-Curtis calculated by vegdist()
```

#### Save Bray-Curtis dissimilarity matrix in a file:
```{r}
  write.table(x = bray.dist.matrix, file = "Output_files/9.potato_median_bray_curtis_sqrt_q1_single_rarefied_to_4622.txt", sep = "\t", row.names = TRUE, col.names = NA)  
...
* subject_id and timepoint interaction =  Could be tested and YES significant, explaining 5% of variance
