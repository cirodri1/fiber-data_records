---
title: "Analysis of Alpha and Beta diversity in R - rarefaction, alpha- and beta- diversity and vizualizations Dahl_2016_V1V2_forward_only-RS4C"
---
...
```{r}
setwd("/Volumes/Seagate/Meta_analysis/Alpha_Beta_analysis/Dahl_2016_V1V2/RS4C/")
```
...
* 2. *Metadata* -  **Dahl_metadata_RS4C.txt** is the Metadata file containing the description of each sample.
...
metadata <- as.data.frame(fread("1.Dahl_metadata_RS4C.tsv"))
...
#### Subset samples in RS4C treatment only based on metadata:
```{r}
#Subset the samples in the RS4C fiber intervention only:
SV.table_RS4C <- SV.table %>% select(metadata$sampleid2)

#Save subset of samples in a file:
write.table(SV.table_RS4C, file = "Output_files/1.SV.table_RS4C.tsv", sep = "\t", row.names = TRUE, col.names = NA)
```
...
SV.table_RS4C <- SV.table_RS4C[, which(colSums(SV.table_RS4C) != 0)]
...
```
##### Results:
  R always uses rows, then columns. So in this case we have and 36 samples and 2803 SVs.
...
```
##### Results:
   We have 929685 reads.


#### To see the number of reads per sample, we simply need to add up the rows on the file:
```{r}
reads.per.sample <- rowSums(SV.table_RS4C) %>% sort()
reads.per.sample

#Save a file with the information of reads per sample:
setwd("/Volumes/Seagate/Meta_analysis/Alpha_Beta_analysis/Dahl_2016_V1V2/RS4C/Output_files")
...
rowSums(SV.table_RS4C) %>% summary()
colSums(SV.table_RS4C) %>% summary()
```
#### Result:
 * The min. number of sequences in a sample are 8135 and the max. is 61044; we can use 8135 for rarefaction depth.
 * The min. value for the presence of an SV is 2 and the max. times an SV is present is 59503.

#### Performing the actual rarefaction 
* Rarefaction depth should be >= 1000 at least, we will use the minimum value of sequence depth which is 8135 
* Round out the averages, so we will not have exactly 8135 sequences in each, but we also will not be keeping the super rare, super low abundance << 1 taxa https://rdrr.io/github/GuillemSalazar/EcolUtils/man/rrarefy.perm.html
```{r, results='hide'}
set.seed(2022) #reproducible results
SV.table.rarefied <- as.data.frame((rrarefy.perm(SV.table_RS4C, sample = 8135, n = 1000, round.out = TRUE)))
...
write.table(SV.table.rarefied, file = "3.RS4C_OTU_filtered_rarefied_8135_ROUNDED.tsv", sep = "\t", row.names = TRUE, col.names = NA)
```
...
write.table(x = merged_alpha, file = "4.RS4C_alpha_metrics_merged_metadata.txt", sep = "\t", row.names = TRUE, col.names = NA) 
...
ggplot (merged_alpha, aes(x= factor (timepoint, level= c("before","after")), y=Shannon)) +
  ...
  labs(title = "Shannon-alpha diversity boxplot / Dahl_2016_V1V2-RS4C", x= "Timepoint in fiber treatment", y= "Shannon alpha diversity", fill="Timepoint") +
...
ggplot (merged_alpha, aes(x= factor (timepoint, level= c("before","after")), y=Simpson)) +
  ...
  scale_fill_hue(limits = c ("before","after")) +
  geom_line(aes(group = `subject_id`), color="grey") +
  labs(title = "Simpson-alpha diversity boxplot / Dahl_2016_V1V2-RS4C", x= "Timepoint in fiber treatment", y= "Simpson alpha diversity", fill="Timepoint") +
  theme_bw (base_family = "Tw Cen MT", base_size = 14) +
  stat_summary(fun = mean, geom = "point", shape=23, size=2.5, fill="black")
```
...
write.table(x = pairs.only.all.alpha, file = "Output_files/6.RS4C_alpha_metrics_merged_metadata_only_pairs.txt", sep = "\t", row.names = TRUE, col.names = NA) 
...
bray.dist <- avgdist(SV.table_RS4C, 
                     sample = 8135, 
...
labs(title = "MDS Dahl_2016_V1V2 RS4C", x= "MDS1", y= "NMDS2", fill="Timepoint") +
...
```{r}
set.seed(2022) #reproducible results
  bray.dist.matrix <- as.data.frame(as.matrix(avgdist(SV.table_RS4C, sample = 8135, meanfun = median, transf = sqrt, iterations = 1000))) #default is Bray-Curtis calculated by vegdist()
```

#### Save Bray-Curtis dissimilarity matrix in a file:
```{r}
  write.table(x = bray.dist.matrix, file = "Output_files/9.RS4C_median_bray_curtis_sqrt_q1_single_rarefied_to_8135.txt", sep = "\t", row.names = TRUE, col.names = NA)  
...
* timepoint (before vs after) =  NOT significant, explaining 0.8% of variance
