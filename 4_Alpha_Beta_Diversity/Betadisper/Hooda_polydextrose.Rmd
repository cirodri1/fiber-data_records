---
title: "Betadisper Hooda_polydextrose"
---
...
```{r}
setwd("/Volumes/Seagate_Cedars/Martiny_Lab/Meta-analysis/Data_analysis/Meta_analysis/Betadisp/Hooda_2012_polydextrose")
```

## __________ Documents needed: __________##

* 1. *beta_matrix* - **9.polydextrose_median_bray_curtis_sqrt_q1_single_rarefied_to_2966.txt** 

* 2. *metadata* - **1.Hooda_metadata_polydextrose.tsv** 
...
beta_matrix <- as.data.frame(read.table("Input/9.polydextrose_median_bray_curtis_sqrt_q1_single_rarefied_to_2966.txt", header = TRUE, row.names=1))


metadata <- as.data.frame(fread("Input/1.Hooda_metadata_polydextrose.tsv", header = TRUE))
```
...
labs(title = "NMDS Hooda_polydextrose", x= "MDS1", y= "MDS2", fill="Timepoint") +
...
#### Save figure:
```{r}
ggsave("Figures/NMDS.Hooda_polydextrose.png", width = 7, height = 5)
```
