---
title: "Analysis of Alpha and Beta diversity in R - rarefaction, alpha- and beta- diversity and vizualizations Baxter_2019_V4_forward_only-inulin"
---
...
```{r}
setwd("/Volumes/Seagate/Meta_analysis/Alpha_Beta_analysis/Baxter_2019_V4/inulin/")
```
...
* 2. *Metadata* -  **Baxter_metadata_inulin.txt** is the Metadata file containing the description of each sample.
...
#### Import metadata: Note: Two samples were dropped due to problematic/repeated metadata SRR6444464
SRR6444370; originally 367 samples, now 365
```{r}
setwd("/Volumes/Seagate/Meta_analysis/Alpha_Beta_analysis/Baxter_2019_V4/Input_files")

metadata <- as.data.frame(fread("1.Baxter_metadata_inulin.tsv"))
...
#### Subset samples in inulin treatment only based on metadata:
```{r}
#Subset the samples in the inulin fiber intervention only:
SV.table_inulin <- SV.table %>% select(metadata$sampleid2)

#Save subset of samples in a file:
write.table(SV.table_inulin, file = "Output_files/1.SV.table_inulin.tsv", sep = "\t", row.names = TRUE, col.names = NA)
```
...
```{r}
SV.table_inulin <- as.data.frame(t(SV.table_inulin))
```

#### Look at the first few rows and columns to ensure data got transposed:
```{r}
SV.table_inulin[1:5,1:5]
```
...
SV.table_inulin <- SV.table_inulin[, which(colSums(SV.table_inulin) != 0)]
...
```
##### Results:
  R always uses rows, then columns. So in this case we have and 365 samples and 3687 SVs.
...
```
##### Results:
   We have 7583457 reads.


#### To see the number of reads per sample, we simply need to add up the rows on the file:
```{r}
reads.per.sample <- rowSums(SV.table_inulin) %>% sort()
reads.per.sample

#Save a file with the information of reads per sample:
setwd("/Volumes/Seagate/Meta_analysis/Alpha_Beta_analysis/Baxter_2019_V4/inulin/Output_files")
...
rowSums(SV.table_inulin) %>% summary()
colSums(SV.table_inulin) %>% summary()
```
#### Result:
 * The min. number of sequences in a sample are 4546 and the max. is 66100; we can use 4546 for rarefaction depth.
 * The min. value for the presence of an SV is 2 and the max. times an SV is present is 866359.

#### Performing the actual rarefaction 
* Rarefaction depth should be >= 1000 at least, we will use the minimum value of sequence depth which is 4546 
* Round out the averages, so we will not have exactly 4546 sequences in each, but we also will not be keeping the super rare, super low abundance << 1 taxa https://rdrr.io/github/GuillemSalazar/EcolUtils/man/rrarefy.perm.html
```{r, results='hide'}
set.seed(2022) #reproducible results
SV.table.rarefied <- as.data.frame((rrarefy.perm(SV.table_inulin, sample = 4546, n = 1000, round.out = TRUE)))
...
write.table(SV.table.rarefied, file = "3.inulin_OTU_filtered_rarefied_4546_ROUNDED.tsv", sep = "\t", row.names = TRUE, col.names = NA)
```
...
write.table(x = merged_alpha, file = "4.inulin_alpha_metrics_merged_metadata.txt", sep = "\t", row.names = TRUE, col.names = NA) 
...
ggplot (merged_alpha, aes(x= factor (timepoint, level= c("before","after")), y=Shannon)) +
  ...
  labs(title = "Shannon-alpha diversity boxplot / Baxter_2019_V4-inulin", x= "Timepoint in fiber treatment", y= "Shannon alpha diversity", fill="Timepoint") +
...
ggplot (merged_alpha, aes(x= factor (timepoint, level= c("before","after")), y=Simpson)) +
  ...
  scale_fill_hue(limits = c ("before","after")) +
  geom_line(aes(group = `subject_id`), color="grey") +
  labs(title = "Simpson-alpha diversity boxplot / Baxter_2019_V4-inulin", x= "Timepoint in fiber treatment", y= "Simpson alpha diversity", fill="Timepoint") +
  theme_bw (base_family = "Tw Cen MT", base_size = 14) +
  stat_summary(fun = mean, geom = "point", shape=23, size=2.5, fill="black")
```
...
write.table(x = merged_alpha_2, file = "5.subsetted_inulin_alpha_metrics_merged_metadata.txt", sep = "\t", row.names = TRUE, col.names = NA) 
...
write.table(x = pairs.only.all.alpha, file = "Output_files/6.inulin_alpha_metrics_merged_metadata_only_pairs.txt", sep = "\t", row.names = TRUE, col.names = NA) 
...
* Are before fiber samples more/less diverse than after fiber samples?
    - ANSWER: Based on the plots, it seems that alpha diversity slightly decreases after fiber treatment
...
bray.dist <- avgdist(SV.table_inulin, 
                     sample = 4546, 
...
labs(title = "MDS Baxter_2019_V4 inulin", x= "MDS1", y= "MDS2", fill="Timepoint") +
...
```{r}
set.seed(2022) #reproducible results
  bray.dist.matrix <- as.data.frame(as.matrix(avgdist(SV.table_inulin, sample = 4546, meanfun = median, transf = sqrt, iterations = 1000))) #default is Bray-Curtis calculated by vegdist()
```

#### Save Bray-Curtis dissimilarity matrix in a file:
```{r}
  write.table(x = bray.dist.matrix, file = "Output_files/9.inulin_median_bray_curtis_sqrt_q1_single_rarefied_to_4546.txt", sep = "\t", row.names = TRUE, col.names = NA)  
...
* timepoint (before vs after) =  YES significant, explaining 0.6% of variance
* subject_id and timepoint interaction =  Could be tested and YES significant, explaining 6% of variance
