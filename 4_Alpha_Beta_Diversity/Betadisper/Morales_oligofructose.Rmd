---
title: "Betadisper Morales_oligofructose"
---
...
```{r}
setwd("/Volumes/Seagate_Cedars/Martiny_Lab/Meta-analysis/Data_analysis/Meta_analysis/Betadisp/Morales_2016_oligofructose")
```

## __________ Documents needed: __________##

* 1. *beta_matrix* - **9.oligofructose_median_bray_curtis_sqrt_q1_forward_rarefied_to_66061.txt**

* 2. *metadata* - **1.Morales_metadata_oligofructose.tsv** 
...
beta_matrix <- as.data.frame(read.table("Input/9.oligofructose_median_bray_curtis_sqrt_q1_forward_rarefied_to_66061.txt", header = TRUE, row.names=1))


metadata <- as.data.frame(fread("Input/1.Morales_metadata_oligofructose.tsv", header = TRUE))
```
...
labs(title = "NMDS Morales_oligofructose", x= "MDS1", y= "MDS2", fill="Timepoint") +
...
#### Save figure:
```{r}
ggsave("Figures/NMDS.Morales_oligofructose.png", width = 7, height = 5)
```
