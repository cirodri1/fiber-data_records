---
title: "Betadisper Tap_dietary_fiber"
---
...
```{r}
setwd("/Volumes/Seagate_Cedars/Martiny_Lab/Meta-analysis/Data_analysis/Meta_analysis/Betadisp/Tap_2015_dietary_fiber")
```

## __________ Documents needed: __________##

* 1. *beta_matrix* - **9.dietary_fiber_median_bray_curtis_sqrt_q1_single_rarefied_to_1021.txt**

* 2. *metadata* - **1.Tap_metadata_dietary_fiber.tsv** 
...
beta_matrix <- as.data.frame(read.table("Input/9.dietary_fiber_median_bray_curtis_sqrt_q1_single_rarefied_to_1021.txt", header = TRUE, row.names=1))


metadata <- as.data.frame(fread("Input/1.Tap_metadata_dietary_fiber.tsv", header = TRUE))
```
...
labs(title = "NMDS Tap_dietary_fiber", x= "MDS1", y= "MDS2", fill="Timepoint") +
...
#### Save figure:
```{r}
ggsave("Figures/NMDS.Tap_dietary_fiber.png", width = 7, height = 5)
```
