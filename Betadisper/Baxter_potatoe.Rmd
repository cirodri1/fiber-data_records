---
title: "Betadisper Baxter_potato"
---
...
```{r}
setwd("/Volumes/Seagate_Cedars/Martiny_Lab/Meta-analysis/Data_analysis/Meta_analysis/Betadisp/Baxter_2019_potatoes")
```

## __________ Documents needed: __________##

* 1. *beta_matrix* - **9.potato_median_bray_curtis_sqrt_q1_single_rarefied_to_4622.txt** 

* 2. *metadata* - **1.baxter_metadata_potatoe.tsv** 
...
beta_matrix <- as.data.frame(read.table("Input/9.potato_median_bray_curtis_sqrt_q1_single_rarefied_to_4622.txt", header = TRUE, row.names=1))


metadata <- as.data.frame(fread("Input/1.baxter_metadata_potato.tsv", header = TRUE))
```
...
labs(title = "NMDS Baxter_potatoe", x= "MDS1", y= "MDS2", fill="Timepoint") +
...
#### Save figure:
```{r}
ggsave("Figures/NMDS.Baxter_potatoe.png", width = 7, height = 5)
```
