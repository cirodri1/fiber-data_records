---
title: "Analysis of Alpha and Beta diversity in R - rarefaction, alpha- and beta- diversity and vizualizations Kovatcheva_2015_V1V2_single_only-bread"
---
...
```{r}
setwd("/Volumes/Seagate/Meta_analysis/Alpha_Beta_analysis/Kovatcheva_2015_V1V2/bread/")
```
...
* 2. *Metadata* -  **Kovatcheva_metadata_bread.txt** is the Metadata file containing the description of each sample.
...
#### Import metadata:
```{r}
setwd("/Volumes/Seagate/Meta_analysis/Alpha_Beta_analysis/Kovatcheva_2015_V1V2/Input_files")

metadata <- as.data.frame(fread("1.Kovatcheva_metadata_bread.tsv"))
...
#### Subset samples in bread treatment only based on metadata (not including white wheat bread because less than 10g of fiber):
```{r}
#Subset the samples in the inulin fiber intervention only:
SV.table_bread <- SV.table %>% select(metadata$sampleid2)
```

#### Save the subsetted data:
```{r}
setwd("/Volumes/Seagate/Meta_analysis/Alpha_Beta_analysis/Kovatcheva_2015_V1V2/bread/Output_files")

#Save subset of samples in a file:
write.table(SV.table_bread, file = "1.SV.table_bread.tsv", sep = "\t", row.names = TRUE, col.names = NA)
```
...
SV.table_bread <- SV.table_bread[, which(colSums(SV.table_bread) != 0)]

SV.combined_bread <- SV.table_bread[, which(colSums(SV.table_bread) != 0)]
```
...
```
##### Results:
  R always uses rows, then columns. So in this case we have and 40 samples and 1368 SVs.
...
rowSums(SV.table_bread) %>% summary()
colSums(SV.table_bread) %>% summary()
```
#### Result:
* The min. number of sequences in a sample is 3642 and the max. is 14653; we can use 3642 for rarefaction depth.
 * The min. value for the presence of an SV is 2 and the max. times an SV is present is 13583.

#### Performing the actual rarefaction 
* Rarefaction depth should be >= 1000 at least, we will use the minimum value of sequence depth which is 3642 
* Round out the averages, so we will not have exactly 3642 sequences in each, but we also will not be keeping the super rare, super low abundance << 1 taxa https://rdrr.io/github/GuillemSalazar/EcolUtils/man/rrarefy.perm.html
```{r, results='hide'}
set.seed(2022) #reproducible results
SV.table.rarefied <- as.data.frame((rrarefy.perm(SV.table_bread, sample = 3642, n = 1000, round.out = TRUE)))
...
write.table(SV.table.rarefied, file = "3.bread_OTU_filtered_rarefied_3642_ROUNDED.tsv", sep = "\t", row.names = TRUE, col.names = NA)
```
...
write.table(x = merged_alpha, file = "4.bread_alpha_metrics_merged_metadata.txt", sep = "\t", row.names = TRUE, col.names = NA) 
...
ggplot (merged_alpha, aes(x= factor (timepoint, level= c("before","after")), y=Shannon)) +
  ...
  labs(title = "Shannon-alpha diversity boxplot / Kovatcheva_2015_V1V2-bread", x= "Timepoint in fiber treatment", y= "Shannon alpha diversity", fill="Timepoint") +
...
ggplot (merged_alpha, aes(x= factor (timepoint, level= c("before","after")), y=Simpson)) +
  ...
  scale_fill_hue(limits = c ("before","after")) +
  geom_line(aes(group = `subject_id`), color="grey") +
  labs(title = "Simpson-alpha diversity boxplot / Kovatcheva_2015_V1V2-bread", x= "Timepoint in fiber treatment", y= "Simpson alpha diversity", fill="Timepoint") +
  theme_bw (base_family = "Tw Cen MT", base_size = 14) +
  stat_summary(fun = mean, geom = "point", shape=23, size=2.5, fill="black")
```
...
write.table(x = pairs.only.all.alpha, file = "Output_files/6.bread_alpha_metrics_merged_metadata_only_pairs.txt", sep = "\t", row.names = TRUE, col.names = NA) 
...
bray.dist <- avgdist(SV.table_bread, 
                     sample = 3642, 
...
labs(title = "MDS Kovatcheva_2015_V1V2 bread", x= "MDS1", y= "NMDS2", fill="Timepoint") +
...
```{r}
set.seed(2022) #reproducible results
  bray.dist.matrix <- as.data.frame(as.matrix(avgdist(SV.table_bread, sample = 3642, meanfun = median, transf = sqrt, iterations = 1000))) #default is Bray-Curtis calculated by vegdist()
```

#### Save Bray-Curtis dissimilarity matrix in a file:
```{r}
  write.table(x = bray.dist.matrix, file = "Output_files/9.bread_median_bray_curtis_sqrt_q1_single_rarefied_to_3642.txt", sep = "\t", row.names = TRUE, col.names = NA)  
...
* timepoint (before vs after) =  NOT significant, explaining 0.6% of variance
