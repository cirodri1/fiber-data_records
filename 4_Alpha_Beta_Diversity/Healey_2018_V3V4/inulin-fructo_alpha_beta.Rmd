---
title: "Analysis of Alpha and Beta diversity in R - rarefaction, alpha- and beta- diversity and vizualizations Healey_2018_V3V4_forward_only-inulin_fructo"
---
...
```{r}
setwd("/Volumes/Seagate/Meta_analysis/Alpha_Beta_analysis/Healey_2018_V3V4/inulin_fructo/")
```
...
* 2. *Metadata* -  **Healey_metadata_inulin_fructo.txt** is the Metadata file containing the description of each sample.
...
#### Import metadata:
```{r}
setwd("/Volumes/Seagate/Meta_analysis/Alpha_Beta_analysis/Healey_2018_V3V4/Input_files")

metadata <- as.data.frame(fread("1.Healey_metadata_inulin_fructo.tsv"))
...
#### Subset samples in inulin_fructo treatment only based on metadata:
```{r}
#Subset the samples in the inulin fiber intervention only:
SV.table_inulin_fructo <- SV.table %>% select(metadata$sampleid2)
```

#### Save the subsetted data:
```{r}
setwd("/Volumes/Seagate/Meta_analysis/Alpha_Beta_analysis/Healey_2018_V3V4/inulin_fructo/Output_files")

#Save subset of samples in a file:
write.table(SV.table_inulin_fructo, file = "1.SV.table_inulin_fructo.tsv", sep = "\t", row.names = TRUE, col.names = NA)
```
...
```{r}
SV.table_inulin_fructo <- as.data.frame(t(SV.table_inulin_fructo))
```

#### Look at the first few rows and columns to ensure data got transposed:
```{r}
SV.table_inulin_fructo[1:5,1:5]
```
...
SV.table_inulin_fructo <- SV.table_inulin_fructo[, which(colSums(SV.table_inulin_fructo) != 0)]

```

#### See the total number of samples and SVs, by looking at the dimensions of the table:
```{r}
dim(SV.table_inulin_fructo)
```
##### Results:
  R always uses rows, then columns. So in this case we have and 68 samples and 2707 SVs.

#### If we now want to know the total number of reads we have we can just count up the total of the SVs sum in table:
```{r}
sum(SV.table_inulin_fructo)
```
##### Results:
   We have 5008418 reads.


#### To see the number of reads per sample, we simply need to add up the rows on the file:
```{r}
reads.per.sample <- rowSums(SV.table_inulin_fructo) %>% sort()
reads.per.sample
...
rowSums(SV.table_inulin_fructo) %>% summary()
colSums(SV.table_inulin_fructo) %>% summary()
```
#### Result:
* The min. number of sequences in a sample is 628 and the max. is 152414; since 628 is too low, we can use 6014 for rarefaction depth- we will lose one sample.
 * The min. value for the presence of an SV is 2 and the max. times an SV is present is 233369.

#### Performing the actual rarefaction 
* Rarefaction depth should be >= 1000 at least, we will use the minimum value of sequence depth which is 6014 
* Round out the averages, so we will not have exactly 6014 sequences in each, but we also will not be keeping the super rare, super low abundance << 1 taxa https://rdrr.io/github/GuillemSalazar/EcolUtils/man/rrarefy.perm.html
```{r, results='hide'}
set.seed(2022) #reproducible results
SV.table.rarefied <- as.data.frame((rrarefy.perm(SV.table_inulin_fructo, sample = 6014, n = 1000, round.out = TRUE)))
...
#### Drop those samples that had under rarefaction threshold (samples SRR6185270):
#https://www.datasciencemadesimple.com/delete-or-drop-rows-in-r-with-conditions-2/
```{r}
 SV.table.rarefied <- SV.table.rarefied[!(row.names(SV.table.rarefied) %in% c('SRR6185270')), ]

sort(rowSums(SV.table.rarefied))
```
#### Results:
We dropped sample SRR6185270

#### Save filtered & rarefied OTU SV.table.rarefied:
```{r}
setwd("/Volumes/Seagate/Meta_analysis/Alpha_Beta_analysis/Healey_2018_V3V4/inulin_fructo/Output_files")

write.table(SV.table.rarefied, file = "3.inulin_fructo_OTU_filtered_rarefied_6014_ROUNDED.tsv", sep = "\t", row.names = TRUE, col.names = NA)
```
...
write.table(x = merged_alpha, file = "4.inulin_fructo_alpha_metrics_merged_metadata.txt", sep = "\t", row.names = TRUE, col.names = NA) 
...
ggplot (merged_alpha, aes(x= factor (timepoint, level= c("before","after")), y=Shannon)) +
  ...
  labs(title = "Shannon-alpha diversity boxplot / Healey_2018_V3V4-inulin_fructo", x= "Timepoint in fiber treatment", y= "Shannon alpha diversity", fill="Timepoint") +
...
ggplot (merged_alpha, aes(x= factor (timepoint, level= c("before","after")), y=Simpson)) +
  ...
  scale_fill_hue(limits = c ("before","after")) +
  geom_line(aes(group = `subject_id`), color="grey") +
  labs(title = "Simpson-alpha diversity boxplot / Healey_2018_V3V4-inulin_fructo", x= "Timepoint in fiber treatment", y= "Simpson alpha diversity", fill="Timepoint") +
  theme_bw (base_family = "Tw Cen MT", base_size = 14) +
  stat_summary(fun = mean, geom = "point", shape=23, size=2.5, fill="black")
```
...
write.table(x = pairs.only.all.alpha, file = "Output_files/6.inulin_fructo_alpha_metrics_merged_metadata_only_pairs.txt", sep = "\t", row.names = TRUE, col.names = NA) 
...
bray.dist <- avgdist(SV.table_inulin_fructo, 
                     sample = 6014, 
...
labs(title = "MDS Healey_2018_V3V4 inulin_fructo", x= "MDS1", y= "MDS2", fill="Timepoint") +
...
```{r}
set.seed(2022) #reproducible results
  bray.dist.matrix <- as.data.frame(as.matrix(avgdist(SV.table_inulin_fructo, sample = 6014, meanfun = median, transf = sqrt, iterations = 1000))) #default is Bray-Curtis calculated by vegdist()
```

#### Save Bray-Curtis dissimilarity matrix in a file:
```{r}
  write.table(x = bray.dist.matrix, file = "Output_files/9.inulin_fructo_median_bray_curtis_sqrt_q1_forward_rarefied_to_6014.txt", sep = "\t", row.names = TRUE, col.names = NA)  
...
```{r}
metadata <- metadata[!(metadata$sampleid2 %in% c('SRR6185270')), ]
...
* timepoint (before vs after) =  YES significant, explaining 1.6% of variance
