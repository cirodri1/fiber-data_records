---
title: "Betadisper Baxter_himaize"
---
...
```{r}
setwd("/Volumes/Seagate_Cedars/Martiny_Lab/Meta-analysis/Data_analysis/Meta_analysis/Betadisp/Baxter_2019_himaize")
```

## __________ Documents needed: __________##

* 1. *beta_matrix* - **9.himaize_median_bray_curtis_sqrt_q1_single_rarefied_to_4891.txt** 

* 2. *metadata* - **1.baxter_metadata_himaize.tsv** 
...
beta_matrix <- as.data.frame(read.table("Input/9.himaize_median_bray_curtis_sqrt_q1_single_rarefied_to_4891.txt", header = TRUE, row.names=1))


metadata <- as.data.frame(fread("Input/1.baxter_metadata_himaize.tsv", header = TRUE))
```
...
labs(title = "NMDS Baxter_himaize", x= "MDS1", y= "MDS2", fill="Timepoint") +
...
#### Save figure:
```{r}
ggsave("Figures/NMDS.Baxter_himaize.png", width = 7, height = 5)
```
