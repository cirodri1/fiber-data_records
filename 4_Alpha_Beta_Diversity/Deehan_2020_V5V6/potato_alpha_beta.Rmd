---
title: "Analysis of Alpha and Beta diversity in R - rarefaction, alpha- and beta- diversity and vizualizations Deehan_2020_V5V6_forward_only-potato"
---
...
```{r}
setwd("/Volumes/Seagate/Meta_analysis/Alpha_Beta_analysis/Deehan_2020_V5V6/potato/")
```
...
* 2. *Metadata* -  **Deehan_metadata_potato.txt** is the Metadata file containing the description of each sample.
...
metadata <- as.data.frame(fread("1.Deehan_metadata_potato.tsv"))
...
#### Subset samples in potato treatment only based on metadata:
```{r}
#Subset the samples in the inulin fiber intervention only:
SV.table_potato <- SV.table %>% select(metadata$sampleid2)
```

#### Save the subsetted data:
```{r}
setwd("/Volumes/Seagate/Meta_analysis/Alpha_Beta_analysis/Deehan_2020_V5V6/potato/Output_files")

#Save subset of samples in a file:
write.table(SV.table_potato, file = "1.SV.table_potato.tsv", sep = "\t", row.names = TRUE, col.names = NA)
```
...
```{r}
SV.table_potato <- as.data.frame(t(SV.table_potato))
```

#### Look at the first few rows and columns to ensure data got transposed:
```{r}
SV.table_potato[1:5,1:5]
```
...
SV.table_potato <- SV.table_potato[, which(colSums(SV.table_potato) != 0)]
...
```
##### Results:
  R always uses rows, then columns. So in this case we have and 50 samples and 1685 SVs.
...
rowSums(SV.table_potato) %>% summary()
colSums(SV.table_potato) %>% summary()
```
#### Result:
* The min. number of sequences in a sample is 18744 and the max. is 67827; we can use 18744 for rarefaction depth
 * The min. value for the presence of an SV is 2 and the max. times an SV is present is 103063

#### Performing the actual rarefaction 
* Rarefaction depth should be >= 1000 at least, we will use the minimum value of sequence depth which is 18744 
* Round out the averages, so we will not have exactly 18744 sequences in each, but we also will not be keeping the super rare, super low abundance << 1 taxa https://rdrr.io/github/GuillemSalazar/EcolUtils/man/rrarefy.perm.html
```{r, results='hide'}
set.seed(2022) #reproducible results
SV.table.rarefied <- as.data.frame((rrarefy.perm(SV.table_potato, sample = 18744, n = 1000, round.out = TRUE)))
...
write.table(SV.table.rarefied, file = "3.potato_OTU_filtered_rarefied_18744_ROUNDED.tsv", sep = "\t", row.names = TRUE, col.names = NA)
```
...
ggplot (merged_alpha, aes(x= factor (fiber_amount2, level= c("<10g",">10g")), y=Shannon)) +
  ...
  labs(title = "Shannon-alpha diversity boxplot / Deehan_2020_V5V6-potato", x= "Timepoint in fiber treatment", y= "Shannon alpha diversity", fill="Timepoint") +
...
ggplot (merged_alpha, aes(x= factor (fiber_amount2, level= c("<10g",">10g")), y=Simpson)) +
  ...
  scale_fill_hue(limits = c ("<10g",">10g")) +
  geom_line(aes(group = `subject_id`), color="grey") +
  labs(title = "Simpson-alpha diversity boxplot / Deehan_2020_V5V6-potato", x= "Timepoint in fiber treatment", y= "Simpson alpha diversity", fill="Timepoint") +
  theme_bw (base_family = "Tw Cen MT", base_size = 14) +
  stat_summary(fun = mean, geom = "point", shape=23, size=2.5, fill="black")
```
...
* Does diversity change after treatments?
    - ANSWER: Based on the mean alpha diversity metrics, it seems that after the intervention the diversity slightly decreases
...
bray.dist <- avgdist(SV.table_potato, 
                     sample = 18744, 
...
labs(title = "MDS Deehan_2020_V5V6 potato", x= "MDS1", y= "MDS2", fill="Timepoint") +
...
```{r}
set.seed(2022) #reproducible results
  bray.dist.matrix <- as.data.frame(as.matrix(avgdist(SV.table_potato, sample = 18744, meanfun = median, transf = sqrt, iterations = 1000))) #default is Bray-Curtis calculated by vegdist()
```

#### Save Bray-Curtis dissimilarity matrix in a file:
```{r}
  write.table(x = bray.dist.matrix, file = "Output_files/9.potato_median_bray_curtis_sqrt_q1_forward_rarefied_to_18744.txt", sep = "\t", row.names = TRUE, col.names = NA)  
...
* subject_id =  YES significant, explaining 85% of variance
* timepoint (before vs after) =  NOT significant, explaining 0.5% of variance (samples were divided by <10gg or >10gg of fiber)
* subject_id and timepoint interaction =  could be tested but NOT significant, explaining 3.7% of variance
