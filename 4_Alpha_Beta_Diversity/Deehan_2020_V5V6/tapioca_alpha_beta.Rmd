---
title: "Analysis of Alpha and Beta diversity in R - rarefaction, alpha- and beta- diversity and vizualizations Deehan_2020_V5V6_forward_only-tapioca"
---
...
```{r}
setwd("/Volumes/Seagate/Meta_analysis/Alpha_Beta_analysis/Deehan_2020_V5V6/tapioca/")
```
...
* 2. *Metadata* -  **Deehan_metadata_tapioca.txt** is the Metadata file containing the description of each sample.

*The files chosen are from the processing of the forward end reads.
...
#### Import metadata:
```{r}
setwd("/Volumes/Seagate/Meta_analysis/Alpha_Beta_analysis/Deehan_2020_V5V6/Input_files")

metadata <- as.data.frame(fread("1.Deehan_metadata_tapioca.tsv"))
...
metadata <- metadata %>%
  convert_as_factor(`fiber_amount2`)
...
#### Subset samples in tapioca treatment only based on metadata:
```{r}
#Subset the samples in the inulin fiber intervention only:
SV.table_tapioca <- SV.table %>% select(metadata$sampleid2)
```

#### Save the subsetted data:
```{r}
setwd("/Volumes/Seagate/Meta_analysis/Alpha_Beta_analysis/Deehan_2020_V5V6/tapioca/Output_files")

#Save subset of samples in a file:
write.table(SV.table_tapioca, file = "1.SV.table_tapioca.tsv", sep = "\t", row.names = TRUE, col.names = NA)
```
...
```{r}
SV.table_tapioca <- as.data.frame(t(SV.table_tapioca))
```

#### Look at the first few rows and columns to ensure data got transposed:
```{r}
SV.table_tapioca[1:5,1:5]
```
...
rowSums(SV.table_tapioca) %>% summary()
colSums(SV.table_tapioca) %>% summary()
```
#### Result:
* The min. number of sequences in a sample is 7157 and the max. is 69285; we can use 7157 for rarefaction depth
 * The min. value for the presence of an SV is 2 and the max. times an SV is present is 67221.

#### Performing the actual rarefaction 
* Rarefaction depth should be >= 1000 at least, we will use the minimum value of sequence depth which is 7157 
* Round out the averages, so we will not have exactly 7157 sequences in each, but we also will not be keeping the super rare, super low abundance << 1 taxa https://rdrr.io/github/GuillemSalazar/EcolUtils/man/rrarefy.perm.html
```{r, results='hide'}
set.seed(2022) #reproducible results
SV.table.rarefied <- as.data.frame((rrarefy.perm(SV.table_tapioca, sample = 7157, n = 1000, round.out = TRUE)))
...
write.table(SV.table.rarefied, file = "3.tapioca_OTU_filtered_rarefied_7157_ROUNDED.tsv", sep = "\t", row.names = TRUE, col.names = NA)
```
...
write.table(x = merged_alpha, file = "4.tapioca_alpha_metrics_merged_metadata.txt", sep = "\t", row.names = TRUE, col.names = NA) 
...
```{r}
ggplot (merged_alpha, aes(x= factor (fiber_amount2, level= c("<10g",">10g")), y=Shannon)) +
  geom_boxplot() +
  geom_point(shape=21, size=2, (aes(fill = fiber_amount2))) +
  scale_fill_hue(limits = c ("<10g",">10g")) +
  geom_line(aes(group = `subject_id`), color="grey") +
  labs(title = "Shannon-alpha diversity boxplot / Deehan_2020_V5V6-tapioca", x= "Timepoint in fiber treatment", y= "Shannon alpha diversity", fill="Timepoint") +
...
```{r}
means.shannon <- aggregate(merged_alpha$Shannon, list(merged_alpha$fiber_amount2), mean)
means.shannon
```
...
```{r}
ggplot (merged_alpha, aes(x= factor (fiber_amount2, level= c("<10g",">10g")), y=Simpson)) +
  geom_boxplot() +
  geom_point(shape=21, size=2, (aes(fill = fiber_amount2))) +
  scale_fill_hue(limits = c ("<10g",">10g")) +
  geom_line(aes(group = `subject_id`), color="grey") +
  labs(title = "Simpson-alpha diversity boxplot / Deehan_2020_V5V6-tapioca", x= "Timepoint in fiber treatment", y= "Simpson alpha diversity", fill="Timepoint") +
  theme_bw (base_family = "Tw Cen MT", base_size = 14) +
  stat_summary(fun = mean, geom = "point", shape=23, size=2.5, fill="black")
```
...
```{r}
means.simpson <- aggregate(merged_alpha$Simpson, list(merged_alpha$fiber_amount2), mean)
means.simpson
```
...
```{r}
merged_alpha_2 <- merged_alpha [ which(merged_alpha$timepoint_id=='before_1' | merged_alpha$timepoint_id=='after_5'),]
```
...
write.table(x = merged_alpha_2, file = "5.subsetted_tapioca_alpha_metrics_merged_metadata.txt", sep = "\t", row.names = TRUE, col.names = NA) 
...
write.table(x = pairs.only.all.alpha, file = "Output_files/6.tapioca_alpha_metrics_merged_metadata_only_pairs.txt", sep = "\t", row.names = TRUE, col.names = NA) 
...
##### Looking at these plots and t-tests, there are a number of questions we want to ask:

* Are before fiber samples more/less diverse than after fiber samples?
    - ANSWER: Based on the plots, it seems that alpha diversity decreases
 
* Does diversity change after treatments?
    - ANSWER: Based on the mean alpha diversity metrics, it seems that after the intervention the diversity decreases
 
* Is the difference in microbiome richness between before and after fiber treatment statistically significant??
    - ANSWER: YES, based on the paired t-tests for Shannon and Simpson alpha diversity metrics.
...
bray.dist <- avgdist(SV.table_tapioca, 
                     sample = 7157, 
...
```{r}
  ...
  geom_point(pch = 21, aes(fill = as.factor(`fiber_amount2`)), size = 10, alpha = 0.7) +
...
labs(title = "MDS Deehan_2020_V5V6 tapioca", x= "MDS1", y= "MDS2", fill="Timepoint") +
...
```{r}
set.seed(2022) #reproducible results
  bray.dist.matrix <- as.data.frame(as.matrix(avgdist(SV.table_tapioca, sample = 7157, meanfun = median, transf = sqrt, iterations = 1000))) #default is Bray-Curtis calculated by vegdist()
```

#### Save Bray-Curtis dissimilarity matrix in a file:
```{r}
  write.table(x = bray.dist.matrix, file = "Output_files/9.tapioca_median_bray_curtis_sqrt_q1_forward_rarefied_to_7157.txt", sep = "\t", row.names = TRUE, col.names = NA)  
...
```{r}
set.seed(2022) #reproducible results
  PERMANOVA1 <- adonis2(bray.dist.matrix~`subject_id`*`fiber_amount2`, data=metadata, method = "bray", by="terms",permutations = 999)
  PERMANOVA1
...
```{r}
set.seed(2022) #reproducible results
 PERMANOVA2 <- adonis2(bray.dist.matrix~`subject_id`*`fiber_amount2`, data=metadata, method = "bray", by= NULL, permutations = 999)
...
```{r}
set.seed(2022) #reproducible results
PERMANOVA3 <-  adonis2(bray.dist.matrix~`subject_id`+`fiber_amount2`, data=metadata, method = "bray", by="terms",permutations = 999)
...
```{r}
set.seed(2022) #reproducible results
PERMANOVA4 <-  adonis2(bray.dist.matrix~`subject_id`+`fiber_amount2`, data=metadata, method = "bray", by=NULL,permutations = 999)
...
* subject_id =  YES significant, explaining 79% of variance
* timepoint (before vs after) =  YES significant, explaining 1% of variance (samples were divided by <10gg or >10gg of fiber)
* subject_id and timepoint interaction =  YES significant, explaining 6.2% of variance
