---
title: "Analysis of Alpha and Beta diversity in R - rarefaction, alpha- and beta- diversity and vizualizations Hooda_2012_V4V6_single_only-corn"
---
...
```{r}
setwd("/Volumes/Seagate/Meta_analysis/Alpha_Beta_analysis/Hooda_2012_V4V6/corn/")
```
...
* 2. *Metadata* -  **Hooda_metadata_corn.txt** is the Metadata file containing the description of each sample.
...
metadata <- as.data.frame(fread("1.Hooda_metadata_corn.tsv"))
...
#### Subset samples in corn treatment only based on metadata:
```{r}
#Subset the samples in the inulin fiber intervention only:
SV.table_corn <- SV.table %>% select(metadata$sampleid2)
```

#### Save the subsetted data:
```{r}
setwd("/Volumes/Seagate/Meta_analysis/Alpha_Beta_analysis/Hooda_2012_V4V6/corn/Output_files")

#Save subset of samples in a file:
write.table(SV.table_corn, file = "1.SV.table_corn.tsv", sep = "\t", row.names = TRUE, col.names = NA)
```
...
SV.table_corn <- SV.table_corn[, which(colSums(SV.table_corn) != 0)]
...
```
##### Results:
  R always uses rows, then columns. So in this case we have and 19 samples and 413 SVs.
...
```{r}
reads.per.sample <- rowSums(SV.table_corn) %>% sort()
reads.per.sample
...
rowSums(SV.table_corn) %>% summary()
colSums(SV.table_corn) %>% summary()
```
#### Result:
* The min. number of sequences in a sample is 3689 and the max. is 10781; we can use 3689 for rarefaction depth.
 * The min. value for the presence of an SV is 2 and the max. times an SV is present is 13857.

#### Performing the actual rarefaction 
* Rarefaction depth should be >= 1000 at least, we will use the minimum value of sequence depth which is 3689 
* Round out the averages, so we will not have exactly 3689 sequences in each, but we also will not be keeping the super rare, super low abundance << 1 taxa https://rdrr.io/github/GuillemSalazar/EcolUtils/man/rrarefy.perm.html
```{r, results='hide'}
set.seed(2022) #reproducible results
SV.table.rarefied <- as.data.frame((rrarefy.perm(SV.table_corn, sample = 3689, n = 1000, round.out = TRUE)))
...
write.table(SV.table.rarefied, file = "3.corn_OTU_filtered_rarefied_3689_ROUNDED.tsv", sep = "\t", row.names = TRUE, col.names = NA)
```
...
write.table(x = merged_alpha, file = "4.corn_alpha_metrics_merged_metadata.txt", sep = "\t", row.names = TRUE, col.names = NA) 
...
ggplot (merged_alpha, aes(x= factor (timepoint, level= c("before","after")), y=Shannon)) +
  ...
  labs(title = "Shannon-alpha diversity boxplot / Hooda_2012_V4V6-corn", x= "Timepoint in fiber treatment", y= "Shannon alpha diversity", fill="Timepoint") +
...
ggplot (merged_alpha, aes(x= factor (timepoint, level= c("before","after")), y=Simpson)) +
  ...
  scale_fill_hue(limits = c ("before","after")) +
  geom_line(aes(group = `subject_id`), color="grey") +
  labs(title = "Simpson-alpha diversity boxplot / Hooda_2012_V4V6-corn", x= "Timepoint in fiber treatment", y= "Simpson alpha diversity", fill="Timepoint") +
  theme_bw (base_family = "Tw Cen MT", base_size = 14) +
  stat_summary(fun = mean, geom = "point", shape=23, size=2.5, fill="black")
```
...
write.table(x = pairs.only.all.alpha, file = "Output_files/6.corn_alpha_metrics_merged_metadata_only_pairs.txt", sep = "\t", row.names = TRUE, col.names = NA) 
...
bray.dist <- avgdist(SV.table_corn, 
                     sample = 3689, 
...
labs(title = "MDS Hooda_2012_V4V6 corn", x= "MDS1", y= "MDS2", fill="Timepoint") +
...
```{r}
set.seed(2022) #reproducible results
  bray.dist.matrix <- as.data.frame(as.matrix(avgdist(SV.table_corn, sample = 3689, meanfun = median, transf = sqrt, iterations = 1000))) #default is Bray-Curtis calculated by vegdist()
```

#### Save Bray-Curtis dissimilarity matrix in a file:
```{r}
  write.table(x = bray.dist.matrix, file = "Output_files/9.corn_median_bray_curtis_sqrt_q1_single_rarefied_to_3689.txt", sep = "\t", row.names = TRUE, col.names = NA)  
...
* subject_id =  YES significant, explaining 75% of variance
* timepoint (before vs after) =  YES significant, explaining 4.4% of variance
