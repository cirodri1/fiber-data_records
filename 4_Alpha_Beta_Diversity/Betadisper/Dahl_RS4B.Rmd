...
```{r}
setwd("/Volumes/Seagate_Cedars/Martiny_Lab/Meta-analysis/Data_analysis/Meta_analysis/Betadisp/Dahl_2016_RS4B")
```

## __________ Documents needed: __________##

* 1. *beta_matrix* - **9.RS4B_median_bray_curtis_sqrt_q1_single_rarefied_to_15957.txt** 

* 2. *metadata* - **1.Dahl_metadata_RS4B.tsv** 
...
beta_matrix <- as.data.frame(read.table("Input/9.RS4B_median_bray_curtis_sqrt_q1_single_rarefied_to_15957.txt", header = TRUE, row.names=1))


metadata <- as.data.frame(fread("Input/1.Dahl_metadata_RS4B.tsv", header = TRUE))
```
...
labs(title = "NMDS Dahl_RS4B", x= "MDS1", y= "MDS2", fill="Timepoint") +
...
#### Save figure:
```{r}
ggsave("Figures/NMDS.Dahl_RS4Be.png", width = 7, height = 5)
```
