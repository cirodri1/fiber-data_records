---
title: "Betadisper Healey_inulin-fructo"
---
...
```{r}
setwd("/Volumes/Seagate_Cedars/Martiny_Lab/Meta-analysis/Data_analysis/Meta_analysis/Betadisp/Healey_2018_inulin_fructo")
```

## __________ Documents needed: __________##

* 1. *beta_matrix* - **9.inulin_fructo_median_bray_curtis_sqrt_q1_forward_rarefied_to_6014.txt** 

* 2. *metadata* - **1.Healey_metadata_inulin_fructo.tsv** 
...
beta_matrix <- as.data.frame(read.table("Input/9.inulin_fructo_median_bray_curtis_sqrt_q1_forward_rarefied_to_6014.txt", header = TRUE, row.names=1))


metadata <- as.data.frame(fread("Input/1.Healey_metadata_inulin_fructo.tsv", header = TRUE))
```
...
labs(title = "NMDS Healey_inulin-fructo", x= "MDS1", y= "MDS2", fill="Timepoint") +
...
#### Save figure:
```{r}
ggsave("Figures/NMDS.Healey_inulin-fructo.png", width = 7, height = 5)
```
