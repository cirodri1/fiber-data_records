---
title: "Analysis of Alpha and Beta diversity in R - rarefaction, alpha- and beta- diversity and vizualizations Rasmussen_2017_V1V3_single_only-psyllium"
---
...
```{r}
setwd("/Volumes/Seagate/Meta_analysis/Alpha_Beta_analysis/Rasmussen_2017_V1V3/psyllium/")
```
...
* 2. *Metadata* -  **Rasmussen_metadata_psyllium.txt** is the Metadata file containing the description of each sample.
...
#### Import metadata:
```{r}
setwd("/Volumes/Seagate/Meta_analysis/Alpha_Beta_analysis/Rasmussen_2017_V1V3/Input_files")

metadata <- as.data.frame(fread("1.Rasmussen_metadata_psyllium.tsv"))
...
SV.table_psyllium <- SV.table %>% select(metadata$sampleid2)
```

#### Save the subsetted data:
```{r}
setwd("/Volumes/Seagate/Meta_analysis/Alpha_Beta_analysis/Rasmussen_2017_V1V3/psyllium/Output_files")

#Save subset of samples in a file:
write.table(SV.table_psyllium, file = "1.SV.table_psyllium.tsv", sep = "\t", row.names = TRUE, col.names = NA)
```
...
```{r}
SV.table_psyllium <- as.data.frame(t(SV.table_psyllium))
```

#### Look at the first few rows and columns to ensure data got transposed:
```{r}
SV.table_psyllium[1:5,1:5]
```
...
SV.table_psyllium <- SV.table_psyllium[, which(colSums(SV.table_psyllium) != 0)]

SV.combined_psyllium <- SV.table_psyllium[, which(colSums(SV.table_psyllium) != 0)]
```

#### See the total number of samples and SVs, by looking at the dimensions of the table:
```{r}
dim(SV.table_psyllium)
```
##### Results:
  R always uses rows, then columns. So in this case we have and 24 samples and 925 SVs.

#### If we now want to know the total number of reads we have we can just count up the total of the SVs sum in table:
```{r}
sum(SV.table_psyllium)
```
##### Results:
   We have 143438 reads.


#### To see the number of reads per sample, we simply need to add up the rows on the file:
```{r}
reads.per.sample <- rowSums(SV.table_psyllium) %>% sort()
reads.per.sample
...
rowSums(SV.table_psyllium) %>% summary()
colSums(SV.table_psyllium) %>% summary()
```
#### Result:
* The min. number of sequences in a sample are 2 and the max. is 12093; since 2 is too low, we can use 1197 for rarefaction depth, we will lose 1 sample.
 * The min. value for the presence of an SV is 2 and the max. times an SV is present is 13438.

#### Performing the actual rarefaction 
* Rarefaction depth should be >= 1000 at least, we will use the minimum value of sequence depth which is 1197 
* Round out the averages, so we will not have exactly 1197 sequences in each, but we also will not be keeping the super rare, super low abundance << 1 taxa https://rdrr.io/github/GuillemSalazar/EcolUtils/man/rrarefy.perm.html
```{r, results='hide'}
set.seed(2022) #reproducible results
SV.table.rarefied <- as.data.frame((rrarefy.perm(SV.table_psyllium, sample = 1197, n = 1000, round.out = TRUE)))
...
#### Drop those samples that had under rarefaction threshold (samples SRR5504436):
#https://www.datasciencemadesimple.com/delete-or-drop-rows-in-r-with-conditions-2/
```{r}
 SV.table.rarefied <- SV.table.rarefied[!(row.names(SV.table.rarefied) %in% c('SRR5504436')), ]

sort(rowSums(SV.table.rarefied))
```
#### Results:
We dropped sample SRR5504436

#### Save filtered & rarefied OTU SV.table.rarefied:
```{r}
setwd("/Volumes/Seagate/Meta_analysis/Alpha_Beta_analysis/Rasmussen_2017_V1V3/psyllium/Output_files")

write.table(SV.table.rarefied, file = "3.psyllium_OTU_filtered_rarefied_1197_ROUNDED.tsv", sep = "\t", row.names = TRUE, col.names = NA)
```
...
write.table(x = merged_alpha, file = "4.psyllium_alpha_metrics_merged_metadata.txt", sep = "\t", row.names = TRUE, col.names = NA) 
...
ggplot (merged_alpha, aes(x= factor (timepoint, level= c("before","after")), y=Shannon)) +
  ...
  labs(title = "Shannon-alpha diversity boxplot / Rasmussen_2017_V1V3-psyllium", x= "Timepoint in fiber treatment", y= "Shannon alpha diversity", fill="Timepoint") +
...
ggplot (merged_alpha, aes(x= factor (timepoint, level= c("before","after")), y=Simpson)) +
  ...
  scale_fill_hue(limits = c ("before","after")) +
  geom_line(aes(group = `subject_id`), color="grey") +
  labs(title = "Simpson-alpha diversity boxplot / Rasmussen_2017_V1V3-psyllium", x= "Timepoint in fiber treatment", y= "Simpson alpha diversity", fill="Timepoint") +
  theme_bw (base_family = "Tw Cen MT", base_size = 14) +
  stat_summary(fun = mean, geom = "point", shape=23, size=2.5, fill="black")
```
...
write.table(x = pairs.only.all.alpha, file = "Output_files/6.psyllium_alpha_metrics_merged_metadata_only_pairs.txt", sep = "\t", row.names = TRUE, col.names = NA) 
...
bray.dist <- avgdist(SV.table_psyllium, 
                     sample = 1197, 
...
labs(title = "MDS Rasmussen_2017_V1V3 psyllium", x= "MDS1", y= "NMDS2", fill="Timepoint") +
...
```{r}
set.seed(2022) #reproducible results
  bray.dist.matrix <- as.data.frame(as.matrix(avgdist(SV.table_psyllium, sample = 1197, meanfun = median, transf = sqrt, iterations = 1000))) #default is Bray-Curtis calculated by vegdist()
```

#### Save Bray-Curtis dissimilarity matrix in a file:
```{r}
  write.table(x = bray.dist.matrix, file = "Output_files/9.psyllium_median_bray_curtis_sqrt_q1_single_rarefied_to_1197.txt", sep = "\t", row.names = TRUE, col.names = NA)  
...
```{r}
metadata <- metadata[!(metadata$sampleid2 %in% c('SRR5504436')), ]
...
* subject_id =  YES significant, explaining 76% of variance
* timepoint (before vs after) =  NOT significant, explaining 2.6% of variance
