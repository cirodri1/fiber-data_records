---
title: "Analysis of Alpha and Beta diversity in R - rarefaction, alpha- and beta- diversity and vizualizations Vandeputte_2017_V4_forward_only-inulin"
---
...
```{r}
setwd("/Volumes/Seagate/Meta_analysis/Alpha_Beta_analysis/Vandeputte_2017_V4/inulin/")
```
...
* 2. *Metadata* -  **venkataraman_metadata_inulin.txt** is the Metadata file containing the description of each sample.
...
#### Import metadata:
```{r}
setwd("/Volumes/Seagate/Meta_analysis/Alpha_Beta_analysis/Vandeputte_2017_V4/Input_files")

metadata <- as.data.frame(fread("1.vandeputte_metadata_inulin.tsv"))
...
SV.combined_inulin <- SV.table_inulin[, which(colSums(SV.table_inulin) != 0)]
```
...
```
##### Results:
  R always uses rows, then columns. So in this case we have and 96 samples and 5232 SVs.
...
setwd("/Volumes/Seagate/Meta_analysis/Alpha_Beta_analysis/Vandeputte_2017_V4/inulin/Output_files")
...
```
#### Result:
 * The min. number of sequences in a sample are 7912 and the max. is 59522; we can use 7912 for rarefaction depth.
 * The min. value for the presence of an SV is 2 and the max. times an SV is present is 79699.

#### Performing the actual rarefaction 
* Rarefaction depth should be >= 1000 at least, we will use the minimum value of sequence depth which is 7912 
* Round out the averages, so we will not have exactly 7912 sequences in each, but we also will not be keeping the super rare, super low abundance << 1 taxa https://rdrr.io/github/GuillemSalazar/EcolUtils/man/rrarefy.perm.html
```{r, results='hide'}
set.seed(2022) #reproducible results
SV.table.rarefied <- as.data.frame((rrarefy.perm(SV.table_inulin, sample = 7912, n = 1000, round.out = TRUE)))
...
write.table(SV.table.rarefied, file = "3.inulin_OTU_filtered_rarefied_7912_ROUNDED.tsv", sep = "\t", row.names = TRUE, col.names = NA)
```
...
ggplot (merged_alpha, aes(x= factor (timepoint, level= c("before","after")), y=Shannon)) +
  ...
  labs(title = "Shannon-alpha diversity boxplot / Vandeputte_2017_V4-inulin", x= "Timepoint in fiber treatment", y= "Shannon alpha diversity", fill="Timepoint") +
...
ggplot (merged_alpha, aes(x= factor (timepoint, level= c("before","after")), y=Simpson)) +
  ...
  scale_fill_hue(limits = c ("before","after")) +
  geom_line(aes(group = `subject_id`), color="grey") +
  labs(title = "Simpson-alpha diversity boxplot / Vandeputte_2017_V4-inulin", x= "Timepoint in fiber treatment", y= "Simpson alpha diversity", fill="Timepoint") +
  theme_bw (base_family = "Tw Cen MT", base_size = 14) +
  stat_summary(fun = mean, geom = "point", shape=23, size=2.5, fill="black")
```
...
labs(title = "MDS Vandeputte_2017_V4 inulin", x= "MDS1", y= "NMDS2", fill="Timepoint") +
...
```{r}
set.seed(2022) #reproducible results
  bray.dist.matrix <- as.data.frame(as.matrix(avgdist(SV.table_inulin, sample = 7912, meanfun = median, transf = sqrt, iterations = 1000))) #default is Bray-Curtis calculated by vegdist()
```

#### Save Bray-Curtis dissimilarity matrix in a file:
```{r}
  write.table(x = bray.dist.matrix, file = "Output_files/9.inulin_median_bray_curtis_sqrt_q1_single_rarefied_to_7912.txt", sep = "\t", row.names = TRUE, col.names = NA)  
...
* subject_id =  YES significant, explaining 83% of variance
* timepoint (before vs after) =  YES significant, explaining 0.7% of variance
