...
```{r}
setwd("/Volumes/Seagate_Cedars/Martiny_Lab/Meta-analysis/Data_analysis/Meta_analysis/Betadisp/Liu_2017_FOS")
```

## __________ Documents needed: __________##

* 1. *beta_matrix* - **9.FOS_median_bray_curtis_sqrt_q1_single_rarefied_to_1929.txt** 

* 2. *metadata* - **1.Liu_metadata_FOS.tsv** 
...
beta_matrix <- as.data.frame(read.table("Input/9.FOS_median_bray_curtis_sqrt_q1_single_rarefied_to_1929.txt", header = TRUE, row.names=1))


metadata <- as.data.frame(fread("Input/1.Liu_metadata_FOS.tsv", header = TRUE))
```
...
labs(title = "NMDS Liu_FOS", x= "MDS1", y= "MDS2", fill="Timepoint") +
...
#### Save figure:
```{r}
ggsave("Figures/NMDS.Liu_FOS.png", width = 7, height = 5)
```
