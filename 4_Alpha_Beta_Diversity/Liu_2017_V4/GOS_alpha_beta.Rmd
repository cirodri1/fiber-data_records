---
title: "Analysis of Alpha and Beta diversity in R - rarefaction, alpha- and beta- diversity and vizualizations Liu_2017_V4_single_only-GOS"
---
...
```{r}
setwd("/Volumes/Seagate/Meta_analysis/Alpha_Beta_analysis/Liu_2017_V4/GOS/")
```
...
* 2. *Metadata* -  **Liu_metadata_GOS.txt** is the Metadata file containing the description of each sample.
...
metadata <- as.data.frame(fread("1.Liu_metadata_GOS.tsv"))
...
SV.table_GOS <- SV.table %>% select(metadata$sampleid2)
```

#### Save the subsetted data:
```{r}
setwd("/Volumes/Seagate/Meta_analysis/Alpha_Beta_analysis/Liu_2017_V4/GOS/Output_files")

#Save subset of samples in a file:
write.table(SV.table_GOS, file = "1.SV.table_GOS.tsv", sep = "\t", row.names = TRUE, col.names = NA)
```
...
SV.table_GOS <- SV.table_GOS[, which(colSums(SV.table_GOS) != 0)]

SV.combined_GOS <- SV.table_GOS[, which(colSums(SV.table_GOS) != 0)]
```
...
```
##### Results:
  R always uses rows, then columns. So in this case we have and 66 samples and 786 SVs.
...
rowSums(SV.table_GOS) %>% summary()
colSums(SV.table_GOS) %>% summary()
```
#### Result:
* The min. number of sequences in a sample is 1465 and the max. is 9349; we can use 1465 for rarefaction depth, we will lose 1 sample.
 * The min. value for the presence of an SV is 2 and the max. times an SV is present is 44155.

#### Performing the actual rarefaction 
* Rarefaction depth should be >= 1000 at least, we will use the minimum value of sequence depth which is 1465 
* Round out the averages, so we will not have exactly 1465 sequences in each, but we also will not be keeping the super rare, super low abundance << 1 taxa https://rdrr.io/github/GuillemSalazar/EcolUtils/man/rrarefy.perm.html
```{r, results='hide'}
set.seed(2022) #reproducible results
SV.table.rarefied <- as.data.frame((rrarefy.perm(SV.table_GOS, sample = 1465, n = 1000, round.out = TRUE)))
...
write.table(SV.table.rarefied, file = "3.GOS_OTU_filtered_rarefied_1465_ROUNDED.tsv", sep = "\t", row.names = TRUE, col.names = NA)
```
...
write.table(x = merged_alpha, file = "4.GOS_alpha_metrics_merged_metadata.txt", sep = "\t", row.names = TRUE, col.names = NA) 
...
ggplot (merged_alpha, aes(x= factor (timepoint, level= c("before","after")), y=Shannon)) +
  ...
  labs(title = "Shannon-alpha diversity boxplot / Liu_2017_V4-GOS", x= "Timepoint in fiber treatment", y= "Shannon alpha diversity", fill="Timepoint") +
...
ggplot (merged_alpha, aes(x= factor (timepoint, level= c("before","after")), y=Simpson)) +
  ...
  scale_fill_hue(limits = c ("before","after")) +
  geom_line(aes(group = `subject_id`), color="grey") +
  labs(title = "Simpson-alpha diversity boxplot / Liu_2017_V4-GOS", x= "Timepoint in fiber treatment", y= "Simpson alpha diversity", fill="Timepoint") +
  theme_bw (base_family = "Tw Cen MT", base_size = 14) +
  stat_summary(fun = mean, geom = "point", shape=23, size=2.5, fill="black")
```
...
write.table(x = pairs.only.all.alpha, file = "Output_files/6.GOS_alpha_metrics_merged_metadata_only_pairs.txt", sep = "\t", row.names = TRUE, col.names = NA) 
...
bray.dist <- avgdist(SV.table_GOS, 
                     sample = 1465, 
...
labs(title = "MDS Liu_2017_V4 GOS", x= "MDS1", y= "NMDS2", fill="Timepoint") +
...
```{r}
set.seed(2022) #reproducible results
  bray.dist.matrix <- as.data.frame(as.matrix(avgdist(SV.table_GOS, sample = 1465, meanfun = median, transf = sqrt, iterations = 1000))) #default is Bray-Curtis calculated by vegdist()
```

#### Save Bray-Curtis dissimilarity matrix in a file:
```{r}
  write.table(x = bray.dist.matrix, file = "Output_files/9.GOS_median_bray_curtis_sqrt_q1_single_rarefied_to_1465.txt", sep = "\t", row.names = TRUE, col.names = NA)  
...
* subject_id =  YES significant, explaining 78% of variance
* timepoint (before vs after) =  YES significant, explaining 1.5% of variance
